...
## Identity
- **Vessel**: Petlog Ai
- **Role**: AI-powered Petlog companion
- **URL**: https://petlog-ai.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/petlog-ai
- **Branch**: master
...
### Deploy
```bash
cd /tmp/petlog-ai && wrangler deploy
```

### Health Check
```bash
curl -s https://petlog-ai.casey-digennaro.workers.dev/health
curl -s https://petlog-ai.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
| /api/memory/transfer | API endpoint |
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=petlog-ai
...
- **vessel.json capabilities**: pet-logging, care-reminders
...
1. Keep the vessel small. ~301 lines is the current size.
